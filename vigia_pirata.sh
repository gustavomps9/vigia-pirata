...
criarDB(){
    ...
    #Verifica se não existe argumentos (diretorias)
	if [ $# -eq 0 ]; then #caso não exista mostra a mensagem de erro e o menu para auxiliar o utilizador
	...
	do
		...
		#O IFS passa a ser o \n (newline)
        #Esta configuração do IFS permite que a leitura de uma variável ou a saída de um comando seja dividida em linhas
		#mesmo se as linhas contenham espaços ou outros caracteres que normalmente seriam tratados como delimitadores pelo IFS padrão.
		OIFS=$IFS; IFS=""
		...
		for pathDir in `find $pathRaiz -type d -not -path "*/.*" 2>/dev/null`; do
			...
			find $pathDir -mindepth 1 -maxdepth 1 -type f -printf "%p\t%M\t%u\t%g\t%s\t%Td/%Tm/%TY\t%TH:%TM\n" 2>/dev/null >> "$tmpDB"
                #%p: Substituído pelo caminho do ficheiro.
                #%M: Substituído pelas permissões do ficheiro.
                #%u: Substituído pelo nome do utilizador proprietário do ficheiro.
                #%g: Substituído pelo nome do grupo proprietário do ficheiro.
                #%s: Substituído pelo tamanho do ficheiro em bytes.
                #%Td/%Tm/%TY: Substituído pela data de modificação do ficheiro no formato dia/mês/ano.
                #%TH:%TM: Substituído pela hora de modificação do ficheiro no formato hora:minuto.
		done
